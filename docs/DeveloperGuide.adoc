...
The undo/redo mechanism is facilitated by `CloneModel` and `StateManager`.
...
The mechanism is supported by a StateManager which stores internally all the states and +
decides when to pop or clear, depending on the command.
There are two stacks that are stored in StateManager internally, the Undo and the Redo stack.
...
Step 1. The user launches the application for the first time. The two internal stacks in `StackManager` will be initialized with the initial Xpire state, and the `currentStatePointer` pointing to that single Xpire state.
