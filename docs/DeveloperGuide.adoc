...
Step 5a. The user then decides to execute the command `list`. Commands that do not modify the Xpire, such as `list`, will usually not call `Model#commitXpire()`, `Model#undoXpire()` or `Model#redoXpire()`. Thus, the `XpireStateList` remains unchanged.

image::UndoRedo/UndoRedoStep5a.png[]

Step 5b. The user executes `clear`, which calls `Model#commitXpire()`. Since the `currentStatePointer` is not pointing at the end of the `XpireStateList`, all Xpire states after the `currentStatePointer` will be purged. We designed it this way because it no longer makes sense to redo the `add n/David ...` command. This is the behavior that most modern desktop applications follow.

image::UndoRedo/UndoRedoStep5b.png[]

Step 5c. The user executes `clear`, which calls `Model#commitXpire()`. Since the `currentStatePointer` is not pointing at the end of the `XpireStateList`, all Xpire states after the `currentStatePointer` will be purged. We designed it this way because it no longer makes sense to redo the `add n/David ...` command. This is the behavior that most modern desktop applications follow.
